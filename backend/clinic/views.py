...
Implements complete CRUD endpoints for Students, Health Records, Consultations,
...
class StudentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Student.objects.prefetch_related('health_records', 'consultations').all()
    ...
    @extend_schema(
        ...
        description="Retrieve all health records for a specific student.",
    ...
    @extend_schema(
        ...
        description="Create a new health record for the specified student.",
    ...
    def health_records(self, request, student_id=None):
        ...
        if request.method == 'GET':
            records = student.health_records.all().order_by('-health_id')
    ...
    @extend_schema(
        methods=['GET'],
        tags=['Students'],
        summary="List consultations for a student",
        description="Retrieve all clinic consultations for a specific student, ordered by visit date descending.",
        responses={200: ConsultationSerializer(many=True)},
    )
    @extend_schema(
        methods=['POST'],
        tags=['Students'],
        summary="Create consultation for a student",
        description="Create a new clinic consultation for the specified student.",
        request=ConsultationSerializer,
        responses={201: ConsultationSerializer},
    )
    @action(detail=True, methods=['get', 'post'], url_path='consultations')
    def consultations(self, request, student_id=None):
        """
        Endpoint: /api/students/<student_id>/consultations/
        - GET: Retrieve all consultations for this student.
        - POST: Create a new consultation for this student.
        """
        student = self.get_object()

        if request.method == 'GET':
            records = student.consultations.all().order_by('-visits', '-consultation_id')
            serializer = ConsultationSerializer(records, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
...
@extend_schema_view(
    ...
    create=extend_schema(
        ...
        description="Create a new health record associated with a student.",
        request=HealthRecordSerializer,
        responses={201: HealthRecordSerializer},
    ),
    update=extend_schema(
    ...
    def get_queryset(self):
        ...
        if search:
            filters = (
                ...
                Q(consultations__consultation__icontains=search)
            )
            if search.isdigit():
...
@extend_schema_view(
    list=extend_schema(
        ...
        description="Retrieve a list of all clinic consultations with optional filtering by student ID or search keyword.",
        parameters=[
            OpenApiParameter(
                name='student_id',
                type=OpenApiTypes.STR,
                location=OpenApiParameter.QUERY,
                description='Filter consultations for a specific student by student ID'
            ),
            OpenApiParameter(
                name='search',
                type=OpenApiTypes.STR,
                location=OpenApiParameter.QUERY,
                description='Search keyword matching consultation notes or student ID'
            ),
        ],
    ),
    retrieve=extend_schema(
        tags=['Consultations'],
        summary="Retrieve a consultation",
        description="Retrieve details of a specific consultation by consultation_id.",
        responses={200: ConsultationSerializer},
    ),
    create=extend_schema(
        tags=['Consultations'],
        summary="Create a consultation",
        description="Create a new clinic consultation associated with a student.",
        request=ConsultationSerializer,
        responses={201: ConsultationSerializer},
    ),
    update=extend_schema(
        ...
        description="Update all fields of an existing consultation.",
        request=ConsultationSerializer,
        responses={200: ConsultationSerializer},
    ),
    partial_update=extend_schema(
        tags=['Consultations'],
        summary="Partially update a consultation",
        description="Partially update one or more fields of an existing consultation.",
        request=ConsultationSerializer,
        responses={200: ConsultationSerializer},
    ),
    destroy=extend_schema(
        ...
        description="Delete an existing consultation by consultation_id.",
        responses={204: None},
    ),
)
class ConsultationViewSet(viewsets.ModelViewSet):
    ...
    Supports complete CRUD operations:
    - GET /api/consultations/ : List all consultations (with optional filtering)
    - POST /api/consultations/ : Create a consultation
    - GET /api/consultations/<consultation_id>/ : Retrieve a single consultation
    - PUT /api/consultations/<consultation_id>/ : Fully update a consultation
    - PATCH /api/consultations/<consultation_id>/ : Partially update a consultation
    - DELETE /api/consultations/<consultation_id>/ : Delete a consultation
    """
    queryset = Consultation.objects.select_related('student').all()
    ...
    def get_queryset(self):
        ...
        if student_id:
            if student_id.isdigit():
                ...
                queryset = queryset.filter(student__student_id__exact=student_id)
        if search:
            filters = Q(consultation__icontains=search)
            if search.isdigit():
                filters |= Q(student__student_id=int(search))
